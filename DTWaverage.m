function [ dist, mat4, refend, refstart ] = DTWaverage( mat1, mat2, cow, q, ce, fe, cw, indel_cost, indel_weight)
...
% cw indicates whether to compute the weight internally (1) or not (0)
...
if (nargin<7)
    ...
    if (nargin<6)
        ...
        if (nargin<5)
            ce = 0 ;
            if (nargin<4)
                q = 0 ; % if no sequence weight specify do not compute average sequence (q==0)
            end
        end
    end
end

%if (fe && size(mat1,1)~=5 && size(mat1,1)~=4) % commented as vector length can be 6 if coverage is saved
...
[dist, mat4, refend, refstart] = DTWave(mat1, mat2, cow, q, ce, fe, cw, indel_cost, indel_weight);
